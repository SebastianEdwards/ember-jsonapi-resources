...
test('#findRelated', function(assert) {
  ...
  let resource   = Ember.getOwner(this).factoryFor('model:post').create(postMock.data);
  ...
  let related = Ember.getOwner(this).factoryFor('model:author').create(authorMock.data);
...
test('#findRelated is called with optional type for the relation', function (assert) {
  ...
  let supervisor = Ember.getOwner(this).factoryFor('model:supervisor').create(supervisorMock.data);
  let employee = Ember.getOwner(this).factoryFor('model:employee').create(employeeMock.data);
...
test('#createResource', function(assert) {
  ...
  let postFactory = Ember.getOwner(this).factoryFor('model:post');
...
test('#updateResource updates (optional) relationships', function(assert) {
  ...
  let author = Ember.getOwner(this).factoryFor('model:comment').create(postMock.included[0]);
  author.set('id', '2');
  this.registry.register('service:authors', adapter.constructor.extend({
    cacheLookup: function () { return author; }
  }));
  let comment = Ember.getOwner(this).factoryFor('model:comment').create(postMock.included[1]);
  comment.set('id', '3');
...
test('#createRelationship uses optional resource type', function (assert) {
  ...
  adapter.serializer = mockSerializer({ relationship: mockRelationSerialized });
  sandbox.stub(adapter, 'fetch', function () { return RSVP.Promise.resolve(null); });
  let resource = Ember.getOwner(this).factoryFor('model:supervisor').create(supervisorMock.data);
